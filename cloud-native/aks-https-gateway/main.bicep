...
var useHttp01 = (domainName != '' && dnsZoneName == '') || useAzureProvidedHostname
...
output domainName string = domainName != ''
  ? domainName
  : (useAzureProvidedHostname ? '${effectiveDnsLabel}.${toLower(location)}.cloudapp.azure.com' : 'aks-https.local')
...
output certificateMode string = useLetsEncrypt
  ? 'letsencrypt'
  : (useAzureProvidedHostname ? 'letsencrypt-azure-http01' : (useHttp01 ? 'letsencrypt-http01' : 'selfsigned'))
