...
The workload is this repository's own `go-hello` Go server, published for `linux/amd64` and `linux/arm64` on GHCR and pinned by OCI image-index digest. Append `/echo` to the HTTPS endpoint to inspect the method, path, headers, forwarded headers, and whether the connection from the Gateway to the origin used TLS:
...
- This repository's `go-hello` server from `ghcr.io/asw101/go-hello`, published for AMD64 and ARM64 and pinned by OCI image-index digest
